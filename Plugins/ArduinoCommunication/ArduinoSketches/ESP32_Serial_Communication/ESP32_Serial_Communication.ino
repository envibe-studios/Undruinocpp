/**
 * ESP32 Serial Communication for Unreal Engine
 *
 * This sketch enables bidirectional text communication between
 * an ESP32 and Unreal Engine via USB Serial connection.
 *
...
void setup() {
  ...
  // Send ready message
  Serial.println("READY:ESP32_Serial");
...
void processCommand(String command) {
  ...
  if (cmd == "PING") {
    ...
    // Read analog value (GPIO 34 is a common ADC pin on ESP32)
    ...
    // Set PWM value using LEDC (ESP32's LED PWM controller)
    // Format: PWM:PIN,VALUE (0-255)
    int commaIndex = params.indexOf(',');
    if (commaIndex > 0) {
      int pin = params.substring(0, commaIndex).toInt();
      int value = params.substring(commaIndex + 1).toInt();

      // ESP32 uses LEDC for PWM - setup channel 0, 5000Hz, 8-bit resolution
      ledcSetup(0, 5000, 8);
      ledcAttachPin(pin, 0);
      ledcWrite(0, value);
 ...
 */
void sendInfo() {
  Serial.println("INFO:DEVICE=ESP32");
  Serial.println("INFO:SDK=" + String(ESP.getSdkVersion()));

  // Get chip ID from MAC address (ESP32 doesn't have getChipId like ESP8266)
  uint64_t chipid = ESP.getEfuseMac();
  char chipIdStr[17];
  snprintf(chipIdStr, sizeof(chipIdStr), "%04X%08X",
           (uint16_t)(chipid >> 32), (uint32_t)chipid);
  Serial.println("INFO:CHIP_ID=" + String(chipIdStr));

  Serial.println("INFO:FLASH_SIZE=" + String(ESP.getFlashChipSize()));
  Serial.println("INFO:FREE_HEAP=" + String(ESP.getFreeHeap()));
  Serial.println("INFO:CPU_FREQ=" + String(ESP.getCpuFreqMHz()) + "MHz");
  Serial.println("INFO:CORES=" + String(ESP.getChipCores()));
}
