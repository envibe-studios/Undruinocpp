...
void UAndySerialSubsystem::HandlePacketDecoded(FName ShipId, const FBenchPacket& Packet)
{
	...
	static int32 AnySrc3 = 0;
	static int32 AnySrc6 = 0;
	static int32 TagSrc3 = 0;
	static int32 TagSrc6 = 0;
	...
	{
		AnySrc3++;
		if (Packet.Type == static_cast<uint8>(EEspMsgType::WeaponTag))
		{
			TagSrc3++;
		}
	}
...

	{
		...
		else if ((Now - LastSummarySeconds) >= 5.0)
		{
			UE_LOG(LogWeaponImu, Warning, TEXT(
				"AndySerial[%s] on wire (5s): imu src3=%d src6=%d other=%d | any src3=%d src6=%d | tag src3=%d src6=%d"),
				*ShipId.ToString(), ImuSrc3, ImuSrc6, ImuSrcOther, AnySrc3, AnySrc6, TagSrc3, TagSrc6);
			if (ImuSrc3 > 0 && ImuSrc6 == 0)
			{
				UE_LOG(LogWeaponImu, Error, TEXT(
					"AndySerial[%s]: ZERO Starboard IMU on USB serial. Andy is not forwarding device 6 — not an Unreal mesh bug."),
					*ShipId.ToString());
			}
			if (ImuSrc6 > 0 && ImuSrc3 == 0)
			{
				UE_LOG(LogWeaponImu, Error, TEXT(
					"AndySerial[%s]: ZERO Port IMU on USB serial (src3 imu=0). Unreal never saw device 3 WeaponImu — not a Port mesh/BP bug."),
					*ShipId.ToString());
				if (AnySrc3 == 0 && TagSrc3 == 0)
				{
					UE_LOG(LogWeaponImu, Error, TEXT(
						"AndySerial[%s]: ZERO packets of ANY type from src=3. Andy is not forwarding Port binary frames (ESP-NOW/peer/ID), or Port is not sending them."),
						*ShipId.ToString());
				}
				else if (TagSrc3 > 0)
				{
					UE_LOG(LogWeaponImu, Error, TEXT(
						"AndySerial[%s]: Port RFID/tag frames ARE on the wire (src3 tag=%d) but IMU is not. Suspect Port IMU sensor/wiring or firmware IMU send path."),
						*ShipId.ToString(), TagSrc3);
				}
			}
			ImuSrc3 = ImuSrc6 = ImuSrcOther = 0;
			AnySrc3 = AnySrc6 = TagSrc3 = TagSrc6 = 0;
			LastSummarySeconds = Now;
		}
	}
