...
public:
	...
	/** Fire now if this component is in bullet mode and the rate-of-fire window is open.
	 *  Used by hardware trigger packets so a shot does not depend on Tick observing bIsFiring. */
	UFUNCTION(BlueprintCallable, Category = "Firing|Control")
	void TryFireIfReady();

	/** Force bullet mode + one rate-limited shot. Used by hardware IMU so fire cannot
	 *  be blocked by inactive Tick, wrong mode, or BP SetFiring order. */
	UFUNCTION(BlueprintCallable, Category = "Firing|Control")
	void ForceFireShot();
...
private:
	...
	/** World time of last FireBullet. Used when SetFiring rising-edge fires without Tick decrementing cooldown. */
	double LastBulletFireTime = -1000.0;
